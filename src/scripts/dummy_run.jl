...
dummy_run_mcts(p::MctsParams) = @set p.num_iters_per_turn = 100
...
function dummy_run_sim(s::SimParams)
  s = @set s.num_games = ConstSchedule(50)
  s = @set s.num_workers = ConstSchedule(20)
  s = @set s.batch_size = ConstSchedule(10)
