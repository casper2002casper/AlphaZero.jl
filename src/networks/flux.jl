...
function Network.forward(nn::GATGraphNeuralNetwork, g)
  ...
  index = vcat([fill(o, sum(A[o, is_machine]) * sum(A[o, is_vehicle])) for o in eachindex(next_op_nodes)]...)
  machine_index = vcat([repeat(findall(A[o, is_machine]), inner=sum(A[o, is_vehicle])) for o in eachindex(next_op_nodes)]...)
  vehicle_index = vcat([repeat(findall(A[o, is_vehicle]), outer=sum(A[o, is_machine])) for o in eachindex(next_op_nodes)]...)
