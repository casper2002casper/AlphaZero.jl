...
struct Trainer
  ...
  function Trainer(gspec, network, samples, params; test_mode=false)
    ...
    dataloader = MLUtils.DataLoader(data; batchsize, partial=false, shuffle=true, parallel=true)
...
function learning_status(tr::Trainer)
  ...
  batches = MLUtils.DataLoader(tr.dataloader.data; batchsize, partial=true, parallel=true)
  reports = []
  ws = []
